...
- **shadcn/ui components**: Located at `apps/web/src/components/ui/`. ALL pages (platform + admin) use shadcn components exclusively. The old `config/platform-theme.ts` and `config/theme.ts` have been **deleted**. Status variant helpers live in `lib/status-variants.ts`. Both sidebars (platform + admin) use the shadcn Sidebar system from `components/ui/sidebar.tsx`. Do NOT re-introduce custom style constants — always use shadcn components directly.
