...
export default function SearchProperties() {
  ...
  const findRegionIdForPoint = useCallback((point: LatLngTuple): number | null => {
    for (const [regionId, shape] of regionShapesRef.current.entries()) {
      if (shape.polygons.some((polygon) => pointInPolygon(point, polygon))) {
        return regionId;
      }
    }
    return null;
  ...
  const renderRegionPolygons = useCallback(() => {
    ...
    if (!map || !L || !regionLayer) return;
      ...
      for (const region of regionShapesRef.current.values()) {
  ...
  const renderDistrictPolygons = useCallback(
    (cityId: number | null) => {
      ...
      districts.forEach((district) => {
        ...
        district.polygons.forEach((ring) => {
          const defaultStyle = {
