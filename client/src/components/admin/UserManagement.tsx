...
export default function UserManagement() {
  ...
  if (!user || !user.roles.includes('WEBSITE_ADMIN')) {
    return (
                ...
                {Object.entries(roleLabels).map(([role, label]) => (
                  <div key={role} className="p-4 border rounded-lg">
                    ...
                    <p className="text-sm text-gray-600 mb-3">
                      {role === 'WEBSITE_ADMIN' && 'صلاحيات كاملة على المنصة'}
                      {role === 'CORP_OWNER' && 'إدارة الشركة والوكلاء'}
                      {role === 'CORP_AGENT' && 'وكيل تحت إشراف شركة'}
                      {role === 'INDIV_AGENT' && 'وكيل مستقل'}
                      {role === 'SELLER' && 'بيع العقارات'}
                      {role === 'BUYER' && 'شراء العقارات'}
                    </p>
