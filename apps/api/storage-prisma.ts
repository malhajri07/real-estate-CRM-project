...
class PrismaStorageSimple {
  ...
  async createPropertyListing(data: any): Promise<any> {
    try {
      // property_id has a DEFAULT in database, so we don't need to provide it
      // Using unchecked input to allow database defaults
      const created = await basePrisma.property_listings.create({
        data: {
          // Don't include property_id - it will be auto-generated by database DEFAULT
          title: data.title,
        ...
        } as any, // Using 'as any' to bypass Prisma type checking for property_id default
      });
      return {
