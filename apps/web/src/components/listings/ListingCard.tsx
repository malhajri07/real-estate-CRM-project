...
export default function ListingCard({ item }: { item: Listing }) {
  ...
  return (
    <div className="ui-surface overflow-hidden group">
      ...
      <div className="p-4">
        <div className="text-sm text-slate-500">{item.city}{item.address ? `، ${item.address}` : ''}</div>
        <div className="font-semibold text-slate-900 mt-1 line-clamp-2">{item.title}</div>
        <div className="flex items-center justify-between mt-2">
          <div className="text-emerald-700 font-bold">{item.price ? `${item.price} ﷼` : ''}</div>
          {item.propertyType && <span className="text-xs px-2 py-1 bg-slate-100 rounded">{item.propertyType}</span>}
        </div>
        <div className="flex gap-2 mt-3">
          <Button onClick={toDetails} className="bg-emerald-600 hover:bg-emerald-700 text-white h-8 px-3 text-xs">{t('listing.details')}</Button>
