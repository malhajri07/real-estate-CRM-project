...
export class ObjectStorageService {
  ...
  async getCSVUploadURL(): Promise<string> {
    ...
    const fullPath = `${privateObjectDir}/csv-uploads/${fileId}.csv`;
  ...
  // Gets the CSV file from the object path for processing.
  async getCSVFile(objectPath: string): Promise<File> {
    let objectEntityDir = this.getPrivateObjectDir();
    if (!objectEntityDir.endsWith("/")) {
      objectEntityDir = `${objectEntityDir}/`;
    }
    const fullObjectPath = `${objectEntityDir}${objectPath}`;
  ...
  // Download CSV file content as string
