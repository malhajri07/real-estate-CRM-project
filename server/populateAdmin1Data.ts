...
export async function populateAdmin1Data() {
  console.log("🚀 بدء إنشاء بيانات إضافية لـ Admin1...");
  
  try {
    // Find Admin1 user
    const admin1User = await db.select().from(users).where(eq(users.email, "admin1@example.com")).limit(1);
    
    if (admin1User.length === 0) {
      console.log("❌ لم يتم العثور على مستخدم Admin1");
      return;
    }
    
    const admin1 = admin1User[0];
    console.log(`✅ تم العثور على مستخدم Admin1: ${admin1.firstName} ${admin1.lastName}`);
    ...
    console.log("👥 إنشاء العملاء المحتملين الإضافيين...");
    for (let i = 0; i < leadsToCreate; i++) {
      ...
      const leadData: any = {
        ...
        ownerId: admin1.id,
        createdBy: admin1.id,
        tenantId: admin1.tenantId || 'tenant-1'
      };
    ...
    // Create properties
    console.log("🏠 إنشاء العقارات الإضافية...");
    ...
    // Create some deals
    console.log("💰 إنشاء الصفقات الإضافية...");
    for (let i = 0; i < 75; i++) {
    ...
    // Create activities
    console.log("📋 إنشاء الأنشطة الإضافية...");
    for (let i = 0; i < 200; i++) {
      const lead = getRandomElement(createdLeads);
    ...
    console.log(`✅ تم إنشاء البيانات الإضافية لـ Admin1 بنجاح:`);
    console.log(`- ${createdLeads.length} عميل محتمل إضافي`);
    console.log(`- ${createdProperties.length} عقار إضافي`);
    console.log(`- 75 صفقة إضافية`);
    console.log(`- 200 نشاط إضافي`);
    ...
    console.error("❌ خطأ في إنشاء البيانات الإضافية:", error);
    throw error;
  }
}
